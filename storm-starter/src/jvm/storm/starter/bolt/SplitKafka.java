 ...
 */
public class SplitKafka extends BaseRichBolt {

  private static final Logger LOG = Logger.getLogger(SplitKafka.class);
 ...
 public static <T> List<List<T>> combination(List<T> values, int size) {
  ...
  public void execute(Tuple tuple) {
	...
	String  timestr = jo.getString("timestamp"); 
	String[] timearray = timestr.split("\\.");
	SimpleDateFormat sdf = new SimpleDateFormat("dd/MMM/yyyy:HH:mm:ss");
        try{
	long timelong = sdf.parse(timearray[0]).getTime()/1000;
	String timestamp = Long.toString(timelong);
	...
	wordlist.add(jo.getString("statusCode"));
	wordlist.add(jo.getString("province"));
	wordlist.add(jo.getString("city"));
	wordlist.add(jo.getString("isp"));
	wordlist.add(jo.getString("backend"));	
	//wordlist.size()
