...
public class SumandRedis extends BaseRichBolt {
  ...
  public SumandRedis(int num_stor,int freq){
    if (num_stor < 3) {
  ...
  public void prepare(Map stormConf, TopologyContext context, OutputCollector collector) {
    ...
    config = new JedisPoolConfig.Builder().setHost("192.168.111.220").setPort(6379).build();
    container = JedisCommandsContainerBuilder.build(config);
    }
  ...
  public void execute(Tuple tuple) {
	...
	String deloldkeytime = tmptime.format(Long.parseLong((key_arr[num_stor-2]).toString())*1000-10800).toString();
	...
	try{
		...
		//LOG.info("get instance from container: "+jedisCommands);
		for (Object obj : objToCounts.keySet()){
			jedisCommands.hset(obj.toString(),humanrealtime,getCount(obj,slot));
			jedisCommands.hdel(obj.toString(),deloldkeytime);
